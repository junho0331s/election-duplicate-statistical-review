...
| Ballot-box and counting-batch chain-of-custody records | Which batch and handling path did the affected ballots follow? | Identical-pair rows concentrate in the same equipment, timestamp, review-table, or batch flow | Batch movement is independent, chronological, and fully reconciled |
...
The audit requested here is therefore not a total and abstract political audit. A limited disclosure for the twelve event rows, especially the Gwangju-Jeonnam five pairs, would narrow which explanation remains among coincidence, display error, reviewed-ballot allocation, data-entry problems, and ballot-box or batch-flow problems.

After raw records are disclosed, the decision path has four levels. First, if the original counting statements or official integrated file change the event-row values and the identical pairs disappear, the core conclusion of this paper should be weakened or withdrawn. Second, if the original values match but every candidate-level change from first-pass sorter output to final publication is fully reconciled by reviewed-ballot records, data-entry logs, and batch records, the statistical-anomaly conclusion remains but the event becomes administratively explained. Third, if the original values match and first-pass-to-final changes repeatedly move multiple pairs toward identical values, the conclusion strengthens from an audit trigger into a causal-investigation stage. Fourth, if original values, first-pass outputs, reviewed-ballot records, input logs, or chain-of-custody records are missing or mutually inconsistent, the chance explanation is not restored; the missing or inconsistent records become the object of independent audit.
