\documentclass[preprint]{../ieie/IEIE}
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{array}
\usepackage{graphicx}
...
\title{Repeated Identical In-District Early-Vote Pairs\\
That Overwhelm the Random-Chance Hypothesis:\\
Statistical Anomalies in the 2026 Korean Local Election\\
and the Need for Raw-Data Audit}
\author{Junho Kim \\ No affiliation \\ e-mail : junhokim0331@gmail.com}
\abstract{
This paper examines whether the repeated identical vote-count pairs observed in in-district early-vote counting units after the June 3, 2026 Korean local election can be explained by ordinary chance. The twelve event rows and six identical vote pairs are rechecked from official National Election Commission election-statistics HTML pages, and 81,701 historical counting-unit rows from 2014 through 2025 are parsed to construct a baseline. Across 51 historical governor contests, the maximum repeated-pair count within a single contest is three. For the Gwangju-Jeonnam five-pair event, the Poisson approximation gives approximately 0.115\% with \(N=393\) and \(K=100,944.8\). If the five observed pairings are treated as designated pairings, the conditional probability that all five match is approximately \(9.54\times10^{-26}\). In 200,000 nonparametric resampling trials, five or more repeated pairs occur zero times, with a rule-of-three 95\% upper bound of about 0.0015\%. The paper also checks early-vote versus election-day two-party vote-share differences in the 2016, 2020, and 2024 National Assembly elections. The conclusion is not a legal finding that a specific act of misconduct has been proven. It is a narrower statistical conclusion: the public data contain anomalies that weaken the random-chance hypothesis and require independent access to raw counting records.\\[2mm]\noindent Keywords: in-district early voting, identical vote pair, election integrity, duplicate probability, Poisson approximation, early-vote deviation, raw-data audit
}
\makeatletter
\renewcommand{\maketitle}{
  \twocolumn[
    \begin{center}
      \vspace*{3mm}
      {\fontsize{16pt}{19.2pt}\selectfont \@title \par}
      \vspace{12mm}
      {\fontsize{10pt}{14pt}\selectfont \@author \par}
      \vspace{8mm}
    \end{center}
  ]
}
...
\begin{enumerate}
\item How often has a pattern of five or more identical top-two candidate vote pairs occurred in the same election, same candidate combination, and same in-district early-vote category in historical official data?
\item Under a simple duplicate-probability model, how likely is the observed repetition?
\item Can the observed pattern be closed as ordinary random coincidence, or does it constitute a statistical anomaly requiring raw-data verification?
\end{enumerate}
...
\begin{enumerate}
\item Group counting units by election, office type, constituency, and vote type.
\item Compute total votes for each candidate within the group.
\item Select the actual first- and second-place candidates by total votes.
\item For each eup, myeon, or dong in-district early-vote counting unit, form the ordered vote pair for these two candidates.
\item Count whether the same vote pair appears in two or more different units.
\end{enumerate}
...
\begin{enumerate}
\item The twelve 2026 event rows are reproduced from official NEC election-statistics HTML pages.
\item Those twelve rows form six identical vote pairs.
\item In the 51 historical governor in-district early-vote contests, the maximum repeated-pair count within a single contest is three.
\item With \(N=393\) and \(K=100,944.8\), the Poisson-tail probability for five or more repeated pairs in Gwangju-Jeonnam is approximately 0.115\%.
\item In 200,000 nonparametric trials sampling 393 units without replacement from the historical governor actual-pair pool, five or more repeated pairs occur zero times.
\item In the 2020 and 2024 National Assembly elections, the Democratic candidate's early-vote two-party share is higher than the election-day two-party share in every analyzable constituency.
\end{enumerate}

The unresolved causal claim is different: whether the identical pairs were produced by natural counting processes, data-entry or display errors, review/reclassification procedures, or another cause. That question requires original counting statements, ballot-sorter first-pass outputs, review-ballot allocation records, and input logs. The detailed claim-to-file mapping is provided in \texttt{evidence\_matrix\_en.md}.
...
\subsection{Conditional Probability for the Five Designated Pairings}
...
\begin{enumerate}
\item Build the pool of 10,322 actual top-two candidate vote pairs from the 2014, 2018, and 2022 governor in-district early-vote data.
\item Draw 393 units without replacement.
\item Count the number of repeated vote pairs in the draw.
\item Repeat this procedure 200,000 times with a fixed random seed.
\end{enumerate}
...
\begin{enumerate}
\item a very low-probability coincidence;
\item an original counting-statement preparation or data-entry error;
\item a display error in the public system;
\item systematic skew in the reviewed-ballot processing stage;
\item actual misconduct.
\end{enumerate}
...
\begin{enumerate}
\item original counting statements for all twelve event rows;
\item candidate-by-candidate first-pass ballot-sorter results for each affected counting unit;
\item the number of reviewed ballots in each affected counting unit;
\item candidate-by-candidate final allocation of the reviewed ballots;
\item invalid votes, abstentions, and third-candidate vote counts;
\item counting-division, sorter, review/aggregation desk, data-entry operator, and time logs;
\item reconciliation between the NEC public-system values and original counting statements;
\item whether party- or candidate-recommended observers raised objections.
\end{enumerate}
...
\[
z =
rac{p_E - p_D}{\sqrt{\hat{p}(1-\hat{p})(1/n_E + 1/n_D)}}
\]
...
This objection was valid against the earliest draft, but it no longer applies in that form. This study extracts the 2026 twelve event rows directly from the NEC election-statistics system's counting-unit result HTML. The reproduction script \texttt{scripts/fetch\_nec\_2026\_duplicate\_cases.py} performs POST queries with election ID \texttt{0020260603}, menu \texttt{VCCP08}, and election type \texttt{metropolitan/provincial governor election}, then parses candidate vote counts from in-district early-vote rows. The result is \texttt{outputs/nec\_2026\_reported\_duplicate\_cases.csv} for the twelve event rows and \texttt{outputs/nec\_2026\_reported\_duplicate\_pairs.csv} for the six identical vote pairs.
...
\begin{enumerate}
\item the reported values for the twelve 2026 identical-vote rows differ from the official original counting statements;
\item the Gwangju-Jeonnam five pairs were not in the same contest, same candidate combination, and same in-district early-vote category;
\item many cases of five or more repeated pairs inside one contest are found in comparable in-district early-vote governor data outside 2014, 2018, and 2022;
\item for all five 2026 pairs, first-pass sorter results, reviewed-ballot allocation, and input logs are independently consistent and document that the identical final values arose naturally through the counting process.
\end{enumerate}
...
The historical baseline analysis in this study is based on public National Election Commission and public-data portal records. The raw files used in the analysis, parsing scripts, probability scripts, and output CSVs are organized inside the submission package. The core outputs can be reproduced by following the procedure in \texttt{README.md}.

The 2026 local-election identical-vote-pair event values were checked by direct extraction from the NEC election-statistics system's counting-unit result HTML. The original HTML cache is included in \texttt{data/nec\_2026\_official\_html/}, and parsed results are included in \texttt{outputs/nec\_2026\_reported\_duplicate\_cases.csv} and \texttt{outputs/nec\_2026\_reported\_duplicate\_pairs.csv}. Once the public-data portal's official integrated XLSX counting file or original counting statements are released, the scripts in this study should be applied under the same event definition to recompute the event values and repetition counts.
...
\begin{itemize}
\item TV Chosun. June 8, 2026. "Identical vote counts appear in twelve places nationwide, including Incheon and Jeonnam; controversy spreads despite the explanation of coincidence." https://news.tvchosun.com/site/data/html\_dir/2026/06/08/2026060890338.html
\item News1/Daum. June 6, 2026. "Why are the vote counts in Songdo 1-dong and Songdo 2-dong the same? Controversy over Park Chan-dae and Yoo Jeong-bok vote counts." https://v.daum.net/v/20260606113452062
\item Hankyoreh. June 8, 2026. "Regarding the same vote counts for Park Chan-dae and Yoo Jeong-bok in Songdo 1-dong and Songdo 2-dong, the election authority says there was no unlawful counting." https://www.hani.co.kr/arti/area/capital/1262526.html
\item Public Data Portal. National Election Commission, vote-counting results for the 6th nationwide local election, June 4, 2014. https://www.data.go.kr/data/15048207/fileData.do
\item Public Data Portal. National Election Commission voting and counting information. https://www.data.go.kr/data/15000900/openapi.do
\item National Election Commission Open Election Data Portal. Public counting-result files for National Assembly, presidential, and nationwide local elections. https://data.nec.go.kr/
\item Public Data Portal. National Election Commission election vote-counting result file data. https://www.data.go.kr/
\item Public Data Portal. Gwangju Metropolitan City community-service-center status. https://www.data.go.kr/data/3082663/fileData.do
\item Jeollanam-do Provincial Government. General status and administrative districts. https://www.jeonnam.go.kr/contentsView.do?menuId=jeonnam0602030000
\end{itemize}

\section*{Appendix D. Reproduction Outputs}

\begin{itemize}
\item Dependency file: \texttt{requirements.txt}
\item Full reproduction script: \texttt{scripts/run\_all.py}
\item Duplicate-analysis script: \texttt{scripts/analyze\_duplicates.py}
\item Historical governor first- and second-place baseline script: \texttt{scripts/analyze\_governor\_actual\_top2.py}
\item Governor nonparametric resampling script: \texttt{scripts/bootstrap\_governor\_duplicates.py}
\item Probability and sensitivity-table reproduction script: \texttt{scripts/probability\_sensitivity.py}
\item Early-vote/election-day check script: \texttt{scripts/analyze\_early\_day\_assembly.py}
\item Additional raw data file: \texttt{data/local2014.xlsx}
\item Existing summary table: \texttt{outputs/duplicate\_summary.csv}
\item Duplicate groups: \texttt{outputs/duplicate\_groups.csv}
\item Duplicate examples: \texttt{outputs/duplicate\_examples.csv}
\item Governor actual first- and second-place baseline summary: \texttt{outputs/governor\_actual\_top2\_summary.csv}
\item Governor contest-level baseline: \texttt{outputs/governor\_actual\_top2\_by\_contest.csv}
\item Governor identical vote-pair cases: \texttt{outputs/governor\_actual\_top2\_duplicates.csv}
\item Governor nonparametric resampling summary: \texttt{outputs/governor\_bootstrap\_summary.csv}
\item Governor nonparametric resampling histogram: \texttt{outputs/governor\_bootstrap\_histogram.csv}
\item Core probability table: \texttt{outputs/probability\_core.csv}
\item Effective pair-space sensitivity table: \texttt{outputs/probability\_k\_sensitivity.csv}
\item Counting-unit-count sensitivity table: \texttt{outputs/probability\_n\_sensitivity.csv}
\item Early-vote/election-day summary: \texttt{outputs/early\_day\_assembly\_summary.csv}
\item Early-vote/election-day constituency-level results: \texttt{outputs/early\_day\_assembly\_twoparty.csv}
\item This paper: \texttt{paper\_statistical\_implausibility\_en.md}
\item English LaTeX manuscript: \texttt{latex/en/main\_en.tex}
\item English PDF manuscript: \texttt{latex/en/main\_en.pdf}
\end{itemize}
