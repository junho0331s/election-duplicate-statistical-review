...
var OPTION = {
	...
	GSGOption : '<option value="2">구시군별</option>',
	SGGOption : '<option value="3">선거구별</option>',
	EMDOption : '<option value="4">읍면동별</option>',
...
function checkPreviousCityCodeForSggCityCode(electionId, electionCode, cityCode) {
	//if (previousCityCode != cityCode) { //2014.04.14 수정
...
//시도코드로 선거구 리스트 호출시 시도 코드 중복체크
function checkPreviousCityCodeIntgForSggCityCode(electionId, electionCode, cityCode) {
	//if (previousCityCode != cityCode) { //2014.04.14 수정
		$('#sggCityCode option').filter(function() {
					$value = $(this).val();
					if ($value > 0) return this;
		...
		parameters.electionCode = electionCode;
		parameters.cityCode = cityCode;
		submitTimer = setTimeout(createSggCityCodesByCityIntg, 100);
	//}
}

//(2026.03.10) 행정통합시도 선거구 가져오기
function createSggCityCodesByCityIntg() {
	var locationStr = '/bizcommon/selectbox/selectbox_getSggCityCodeByCityIntgJson.json';
	var params = {
...
// 시도코드로 구시군 리스트 호출시 시도 코드 중복체크
function checkPreviousCityCodeForTownCode2(electionId, cityCode) {
	if (previousCityCode != cityCode) {
		$('#townCode option').filter(function() {
					$value = $(this).val();
					if ($value > 0) return this;
...
/* 
  	구시군 코드로 선거구 리스트 호출시 구시군 코드 중복체크
	(2020.3.19. 추가) 21대 국회의원선거 한정으로 사용한다.
	개표단위별 개표 결과에서 전주시 덕진구를 표시하기위해 사용
	
		주의
		 - 21대 이후에는 vccp08.jsp에서 checkPreviousCityCodeForSgCode를 다시 사용한다.
		 - 21대 이후에는 vccp08.jsp에서 criteria_gp.jsp, criteria_all.jsp를 다시 include하게 수정한다. 
*/ 
function checkPreviousTownCodeForSggTownCodeForGp(electionId, electionCode, townCode)
{	
	// 구시군 선거구 가져오기
	function createSggTownCodes() {
		...
		var locationStr = '/bizcommon/selectbox/selectbox_getSggTownCodeJsonForGp.json';
		$.getJSONTE(locationStr, params, function(response, status) {
...
// (2011.5.9. 추가) [선거별]선거코드로 시도리스트 호출시 시도 코드 중복체크
...
function checkPreviousCityCodeIntgForTownCodeBySg(electionId, electionCode, cityCode) {
	//if (previousCityCode != cityCode) { //2014.04.14 수정
		$('#townCode option').filter(function() {
					$value = $(this).val();
					if ($value > 0) return this;
		...
		parameters.electionCode = electionCode;
		parameters.cityCode = cityCode;
		submitTimer = setTimeout(createTownCodesByCityIntgSg, 100);
	//}
}

//(2026.03.10) 행정통합시도 구시군목록
function createTownCodesByCityIntgSg() {
	var locationStr = '/bizcommon/selectbox/selectbox_townCodeByCityIntgSgJson.json';
	var params = {
		electionId : parameters.electionId,
		electionCode : parameters.electionCode,
		cityCode : parameters.cityCode
...
개표단위별 개표 결과에서 전주시 덕진구를 표시하기위해 사용
	...
	주의
	 ...
	 - 21대 이후에는 vccp08.jsp에서 criteria_gp.jsp, criteria_all.jsp를 다시 include하게 수정한다. 
*/ 
function checkPreviousCityCodeForTownCodeBySgForGp(electionId, electionCode, cityCode)
{
	function createTownCodesBySg()
	{
		var locationStr = '/bizcommon/selectbox/selectbox_townCodeBySgJsonForGp.json';
		var params = {
			electionId : parameters.electionId,
			electionCode : parameters.electionCode,
			cityCode : parameters.cityCode
...
//행정구역통합 시도코드
function checkPreviousCityCodeDIForSgCode(electionId, electionCode, cityCode) {
	//if (previousElectionCode != electionCode) {
		$('#cityCode option').filter(function() {
					$value = $(this).val();
					if ($value > 0) return this;
		...
		parameters.electionCode = electionCode;
		parameters.cityCode = cityCode;
		submitTimer = setTimeout(createCityCodesDIBySg, 100);
	//}
}

// 2026.02 행정구역통합 시도코드
function createCityCodesDIBySg() {
	var locationStr = '/bizcommon/selectbox/selectbox_cityCodeDIBySgJson.json';
	var params = {
		electionId : parameters.electionId,
		electionCode : parameters.electionCode
	};
...
// (2011.5.9. 추가) [선거별] 시도 > 위원회 리스트 가져오기
function createTownCodesBySg() {
...
//(2012.12.21. 추가) [선거별]선거코드로 시도리스트 호출시 시도 코드 중복체크
...
//(2012.12.21. 추가) [역대선거별] 시도 리스트 가져오기
...
//(2026.03.19 추가) [선거별]선거코드로 시도 호출(행정통합)
function checkPreviousCityIntgCodeForSgCode_Old(electionCode, cityCode, subElectionCode) {
	//if (previousElectionCode != electionCode) {
		$('#cityCode option').filter(function() {
					$value = $(this).val();
					if ($value > 0) return this;
		...
		parameters.cityCode = cityCode;
		submitTimer = setTimeout(createCityIntgCodesBySg_Old, 100);
	//}
}

//(2026.03.19 추가) [역대선거별] 행정통합 시도 리스트 가져오기
function createCityIntgCodesBySg_Old() {
	var locationStr = '/bizcommon/selectbox/selectbox_cityIntgCodeBySgJson_Old.json';
	var params = {
			electionId : '0000000000',
			subElectionCode : parameters.subElectionCode,
			electionCode : parameters.electionCode
...
//(2021.07.12. 추가)[재보궐선거] [선거별]선거코드로 시도리스트 호출
function checkPreviousCityCodeForSgCode_JAEBO_Old(electionCode, cityCode, subElectionCode) {
		...
		parameters.cityCode = cityCode;
		submitTimer = setTimeout(createCityCodesBySg_JAEBO_Old, 100);
}

//(2021.07.12. 추가)[재보궐선거]  [역대선거별] 시도 리스트 가져오기
function createCityCodesBySg_JAEBO_Old() {
	var locationStr = '/bizcommon/selectbox/selectbox_cityCodeBySgJson_JAEBO_Old.json';
	var params = {
			electionId : '0000000000',
			subElectionCode : parameters.subElectionCode,
			electionCode : parameters.electionCode
...
//(2012.12.25. 추가) [구역대선거별] 선거코드로 시도리스트 호출시 시도 코드 중복체크
...
//(2012.12.25. 추가) [구역대선거별] 시도 리스트 가져오기
...
//(2012.01.04. 추가) [역대선거] 선거코드로 하위선거타입 조회
...
// (2021.06.03. 추가) [역대선거] 대통령선거일경우, 하위선거종류 자동으로 대통령선거 선택 + 다음 콤보박스 자동으로 조회
function checkSubElectionTypeForSgCode_Old_DS_nextCombo(electionCode, electionType, delProportion) {
	$('#electionCode option').filter(function() {
				$value = $(this).val();
				if ($value > 0) return this;
			}).remove();
	...
	parameters.electionType = electionType;
	parameters.delProportion = delProportion;
	submitTimer = setTimeout(createSubElectionTypeForSgCode_Old_DS_nextCombo, 100);
}

// (2021.06.03. 추가) [역대선거] 대통령선거일경우, 하위선거종류 자동으로 대통령선거 선택 + 다음 콤보박스 자동으로 조회
function createSubElectionTypeForSgCode_Old_DS_nextCombo() {
	var locationStr = '/bizcommon/selectbox/selectbox_getSubElectionTypeJson.json';
	var params = {
			electionId : '0000000000',
			electionCode : parameters.electionCode,
...
//(2021.06.03. 추가) [역대선거] 대통령선거일경우, 하위선거종류 자동으로 대통령선거 선택
function checkSubElectionTypeForSgCode_Old_DS(electionCode, electionType, delProportion) {
	$('#electionCode option').filter(function() {
				$value = $(this).val();
				if ($value > 0) return this;
			}).remove();
	...
	parameters.electionType = electionType;
	parameters.delProportion = delProportion;
	submitTimer = setTimeout(createSubElectionTypeForSgCode_Old_DS, 100);
}

// (2021.06.03. 추가) [역대선거] 대통령선거일경우, 하위선거종류 자동으로 대통령선거 선택
function createSubElectionTypeForSgCode_Old_DS() {
	var locationStr = '/bizcommon/selectbox/selectbox_getSubElectionTypeJson.json';
	var params = {
			electionId : '0000000000',
			electionCode : parameters.electionCode,
...
//(2012.01.19. 추가) [역대선거] 시도코드로 선거구 리스트 호출시 시도 코드 중복체크
...
//(2012.01.19. 추가) [역대선거] 시도 선거구 가져오기
...
//(2012.01.19. 추가) [구역대선거] 시도코드로 선거구 리스트 호출시 시도 코드 중복체크
...
//(2012.01.19. 추가) [구역대선거] 시도 선거구 가져오기
...
//(2012.01.19. 추가) [역대선거]  구시군 코드로 선거구 리스트 호출시 구시군 코드 중복체크
...
//(2012.01.19. 추가) [역대선거] 구시군 선거구 가져오기
function createSggTownCodes_Old() {
	...
	$.getJSONTE(locationStr, params, function(response, status) {
				...
				if(typeof parameters.cityCode != "undefined" && parameters.cityCode != "")
					previousCityCode = parameters.cityCode;
				previousTownCode = parameters.townCode;
				settingYN();
			});
...
//(2012.01.19. 추가) [구역대선거] 구시군 코드로 선거구 리스트 호출시 구시군 코드 중복체크
...
//(2012.01.19. 추가) [구역대선거] 구시군 선거구 가져오기
...
//(2012.01.24. 추가) [역대선거] //선거구 코드로 구시군 위원회 리스트 호출시 선거구 코드 중복 체크
...
//(2012.01.24. 추가) [역대선거] //선거구에 속한 구시군위원회 리스트 가져오기
...
//(2012.01.30. 추가) [역대선거] [선거별]시도코드로 구시군 리스트 호출시 시도 코드 중복체크
...
//(2026.03.19. 추가) [역대선거] [선거별]행정통합 시도코드로 구시군 리스트 호출시 시도 코드 중복체크
function checkPreviousCityIntgCodeForTownCodeBySg_Old(electionId, electionCode, cityCode, subElectionCode) {
	...
	if (previousCityCode != cityCode) {
		...
		parameters.cityCode = cityCode;
		parameters.subElectionCode = subElectionCode;
		submitTimer = setTimeout(createTowIntgnCodesBySg_Old, 100);
	}
}

//(2026.03.19. 추가) 시도 > 위원회 리스트 가져오기
function createTowIntgnCodesBySg_Old() {
	var locationStr = '/bizcommon/selectbox/selectbox_townIntgCodeBySgJson_Old.json';
	var params = {
		electionId : parameters.electionId,
		electionCode : parameters.electionCode,
		cityCode : parameters.cityCode,
...
//(2024.10.14. 추가) [역대선거] [사전투표소현황, 선거일투표소현황] 화성시갑/을 위원회 화성시로 통합 조회
function checkPreviousCityCodeForTownCodeBySg_Old_Wiw(electionId, electionCode, cityCode, subElectionCode) {
	...
	if (previousCityCode != cityCode) {
		...
		parameters.cityCode = cityCode;
		parameters.subElectionCode = subElectionCode;
		submitTimer = setTimeout(createTownCodesBySg_Old_Wiw, 100);
	}
}

//(2012.01.30. 추가) 시도 > 위원회 리스트 가져오기
function createTownCodesBySg_Old_Wiw() {
	var locationStr = '/bizcommon/selectbox/selectbox_townCodeBySgJson_Old_Wiw.json';
	var params = {
		electionId : parameters.electionId,
		electionCode : parameters.electionCode,
		cityCode : parameters.cityCode,
...
//(2021.07.12. 추가) [역대선거][재보궐선거] [선거별]시도코드로 구시군 리스트 호출시 시도 코드 중복체크 제외
function checkPreviousCityCodeForTownCodeBySg_JAEBO_Old(electionId, electionCode, cityCode, subElectionCode) {
		$('#townCode option').filter(function() {
					$value = $(this).val();
					if ($value > 0) return this;
				}).remove();
		...
		parameters.cityCode = cityCode;
		parameters.subElectionCode = subElectionCode;
		submitTimer = setTimeout(createTownCodesBySg_JAEBO_Old, 100);
}

//(2021.07.12. 추가) [역대선거][재보궐선거] 시도 > 위원회 리스트 가져오기
function createTownCodesBySg_JAEBO_Old() {
	var locationStr = '/bizcommon/selectbox/selectbox_townCodeBySgJson_JAEBO_Old.json';
	var params = {
		electionId : parameters.electionId,
		electionCode : parameters.electionCode,
		cityCode : parameters.cityCode,
...
//(2021.08.17. 추가) [역대선거][재보궐선거] [선거별] 구시군 리스트 조회, 개표현황(읍면동/투표구별)
function checkPreviousCityCodeForAllTownCodeBySg_JAEBO_Old(electionId, electionCode, cityCode, subElectionCode) {
		$('#townCode option').filter(function() {
					$value = $(this).val();
					if ($value > 0) return this;
				}).remove();
		...
		parameters.cityCode = cityCode;
		parameters.subElectionCode = subElectionCode;
		submitTimer = setTimeout(createAllTownCodesBySg_JAEBO_Old, 100);
}

//(2021.07.12. 추가) [역대선거][재보궐선거] 시도 > 위원회 리스트 가져오기
function createAllTownCodesBySg_JAEBO_Old() {
	var locationStr = '/bizcommon/selectbox/selectbox_AllTownCodeBySgJson_JAEBO_Old.json';
	var params = {
		electionId : parameters.electionId,
		electionCode : parameters.electionCode,
		cityCode : parameters.cityCode,
...
//(2012.01.30. 추가) [구역대선거] [선거별]시도코드로 구시군 리스트 호출시 시도 코드 중복체크
...
//(2018.08.21. 추가) 선거년도로 구분 리스트 호출시 코드 중복체크
function checkPreviousSgYearPartBySgYear(electionYear) {
	$('#electionName option').filter(function() {
				$value = $(this).val();
				if ($value > 0) return this;
			}).remove();

	clearTimeout(submitTimer);
	parameters.electionYear = electionYear;
	submitTimer = setTimeout(createSgYearPart, 100);
}

//(2018.08.21. 추가) 선택된 재보궐 선거년도로 구분값 가져오기
function createSgYearPart() {
	var params = {
			electionYear : parameters.electionYear
	};

	var locationStr = '/bizcommon/selectbox/selectbox_getSgYearPartCodeJson.json';
	$.getJSONTE(locationStr, params, function(response, status) {
		var electionNameList = response;
		var electionNameEl = $('#electionName');
		// sggTownCode 옵션 추가
		for (var i = 0; i < electionNameList.length; i++) {
			var option = $('<option>').attr('value',
					electionNameList[i].CODE)
					.html(electionNameList[i].NAME);
			option.appendTo(electionNameEl);
		}
		if(typeof parameters.electionYear != "undefined" && parameters.electionYear != "")
			previousElectionYear = parameters.electionYear;
		settingYN();
	});
}

//(2018.08.22. 추가) 선거 구분으로 선거종류 리스트 호출시 코드 중복체크
function checkPreviousSubSgBySgYearPart(electionName) {
	//if (previousTownCode != townCode) {
		$('#subElectionCode option').filter(function() {
					$value = $(this).val();
					if ($value > 0) return this;
				}).remove();
		...
		clearTimeout(submitTimer);
		parameters.electionName = electionName;
		submitTimer = setTimeout(createSubSg, 100);
	//}
}

//(2018.08.22. 추가) 선택된 구분으로 선거종류 가져오기
function createSubSg() {
	var params = {
			electionName : parameters.electionName
	};

	var locationStr = '/bizcommon/selectbox/selectbox_getSubSgCodeJson.json';
	$.getJSONTE(locationStr, params, function(response, status) {
		var subElectionCodeList = response;
		var subElectionCodeEl = $('#subElectionCode');
		// sggTownCode 옵션 추가
		for (var i = 0; i < subElectionCodeList.length; i++) {
			var option = $('<option>').attr('value', subElectionCodeList[i].CODE)
					.html(subElectionCodeList[i].NAME);
			option.appendTo(subElectionCodeEl);
		}
		if(typeof parameters.subElectionCode != "undefined" && parameters.subElectionCode != "")
			previousSubElectionCode = parameters.subElectionCode;
		settingYN();
	});
}

// (2018.08.31. 추가) [선거별] 선거코드로 시도리스트 호출시 시도 코드 중복체크(unielesys)
function checkPreviouscityCodeForSubSgCode(electionId, electionYear, electionName, subElectionCode) {
	$('#cityCode option').filter(function() {
				$value = $(this).val();
				if ($value > 0) return this;
			}).remove();
	...
	if(typeof electionYear != "undefined" && electionYear != null && electionYear != "") {
		parameters.electionYear = electionYear;
	} else {
		parameters.electionYear = null;
	}
	
	if(typeof electionName != "undefined" && electionName != null && electionName != "") {
		parameters.electionName = electionName;
	} else {
		parameters.electionName = null;
	}
	
	if(typeof subElectionCode != "undefined" && subElectionCode != null && subElectionCode != "") {
		parameters.subElectionCode = subElectionCode;
	} else {
		parameters.subElectionCode = null;
	}
	
	submitTimer = setTimeout(createCityCodesBySubSg, 100);
}

//(2018.08.31. 추가) [선거별] 시도 리스트 가져오기
function createCityCodesBySubSg() {
	var params = {
		electionId : parameters.electionId,
		electionYear : parameters.electionYear,
		electionName : parameters.electionName,
	...
	var locationStr = '/bizcommon/selectbox/selectbox_getCityCodeSubSgCodeJson.json';
	$.getJSONTE(locationStr, params, function(response, status) {
				var cityCodeList = response;
				var cityCodeEl = $('#cityCode');
				// cityCode 옵션 추가
...
//(2019.12.05. 추가) [역대선거] 선거코드로 하위선거타입 조회 (임기중 당선인)
function checkSubElectionTypeForSgCodeByElection_Old(electionCode, electionType, delProportion) {
	$('#electionCode option').filter(function() {
				$value = $(this).val();
				if ($value > 0) return this;
			}).remove();
	...
	parameters.electionType = electionType;
	parameters.delProportion = delProportion;
	submitTimer = setTimeout(createSubElectionTypeForSgCodeByElection_Old, 100);
}
var oldElectedPhotoUseYnList = [];
//(2019.12.05. 추가) [역대선거] 선거코드로 하위선거타입 조회 (임기중 당선인)
function createSubElectionTypeForSgCodeByElection_Old() {
	var locationStr = '/bizcommon/selectbox/selectbox_getSubElectionTypeByElectedJson.json';
	var params = {
			electionId : '0000000000',
			electionCode : parameters.electionCode,
			electionType : parameters.electionType
	...
	$.getJSONTE(locationStr, params, function(response, status) {
				var electionCodeList = response;
				var electionCodeEl = $('#electionCode');
				oldElectedPhotoUseYnList = response;
				// electionCode 옵션 추가
				//alert(parameters.delProportion);
				for (var i = 0; i < electionCodeList.length; i++) {
					var option = $('<option>').attr('value',
