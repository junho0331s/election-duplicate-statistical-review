...
| `outputs/pre_submission_audit.json` | JSON audit summary | Machine-readable result of the 13 pre-submission audit checks. | `status`, `check_count`, `scope`, `checks` |
| `outputs/submission_integrity_report.md` | One integrity report | Human-readable final integrity summary for PDFs, core checks, claim-boundary checks, objection coverage, audit checks, and key reproducible numbers. | Markdown sections |
| `outputs/submission_integrity_report.json` | JSON integrity report | Machine-readable final integrity summary excluding the final ZIP self-hash. | `status`, `core_claims_check_count`, `claim_boundary_audit_check_count`, `objection_coverage_audit_check_count`, `pre_submission_audit_check_count`, `korean_pdf`, `english_pdf`, `key_claims` |
