...
| `outputs/probability_designated_pairs.csv` | One designated-pair conditional probability scenario | Conditional probability that the observed five Gwangju-Jeonnam pairings would all match if treated as pre-designated pairings. | `case`, `k_space`, `designated_pairs`, `probability`, `probability_percent`, `reciprocal` |
...
| `outputs/public_discussion_claims_audit.csv` | One public-discussion auxiliary-claim check row | Rechecks checkable auxiliary claims from public collaborative pages or online discussions against official data only. | `election`, `city`, `district`, `polling_station`, `lee_jae_myung`, `yoon_suk_yeol`, `ballots_cast`, `registered_voters` |
| `outputs/public_discussion_claims_audit.json` | JSON public-discussion auxiliary-claim check summary | Machine-readable verification that the 2022 Bisan 1-dong polling-station pair claim is confirmed in the official presidential election file. | `status`, `row_count`, `confirmed_pair`, `classification`, `failures` |
...
| `outputs/core_claims_verification.json` | JSON verification summary | Machine-readable verification result for the same 47 core values. | `status`, `check_count`, `scope`, `checks` |
| `outputs/statistical_robustness_audit.csv` | One statistical robustness check | Spreadsheet-readable audit of Poisson, exact-collision, sensitivity, and resampling outputs. | `check`, `expected`, `actual`, `status` |
| `outputs/statistical_robustness_audit.json` | JSON statistical robustness audit summary | Machine-readable verification that the key statistical conclusion is not dependent on one approximation only. | `status`, `check_count`, `scope`, `checks` |
| `outputs/video_source_exclusion_audit.csv` | One manuscript-facing file | Spreadsheet-readable audit that the submission does not rely on video-platform or subtitle-based informal evidence. | `file`, `video_url_count`, `informal_marker_count`, `status` |
| `outputs/video_source_exclusion_audit.json` | JSON video-source exclusion audit summary | Machine-readable verification that 25 manuscript-facing files contain no informal video-platform URLs or subtitle-dependence markers. | `status`, `check_count`, `checked_files`, `failures` |
...
| `outputs/claim_boundary_audit.csv` | One claim-boundary check | Spreadsheet-readable audit that the package states a statistical anomaly and raw-data audit request without unsupported legal finality. | `check`, `file`, `expected`, `actual`, `status` |
| `outputs/claim_boundary_audit.json` | JSON claim-boundary audit summary | Machine-readable verification of overclaim absence, falsifiability language, raw-record requirements, and post-disclosure decision levels in manuscripts, memos, and PDFs. | `status`, `check_count`, `scope`, `checks` |
| `outputs/objection_coverage_audit.csv` | One objection-coverage check | Spreadsheet-readable audit that expected reviewer objections are addressed in the paper and support memos. | `objection`, `file`, `expected`, `actual`, `status` |
| `outputs/objection_coverage_audit.json` | JSON objection-coverage audit summary | Machine-readable verification of coverage for one-duplicate, 2014 inclusion, three-versus-five, pre-designated five-pair probability interpretation, look-elsewhere, heterogeneity, self-selection, rare-event, official-file, benign-alternative, chain-of-custody raw-record objections, and the post-disclosure decision tree. | `status`, `check_count`, `scope`, `checks` |
...
| `outputs/pre_submission_audit.json` | JSON audit summary | Machine-readable result of the 17 pre-submission audit checks. | `status`, `check_count`, `scope`, `checks` |
...
| If the five Gwangju-Jeonnam pairings are treated as pre-designated pairings, the conditional probability that all five match is about \(9.54\times10^{-26}\). | `outputs/probability_designated_pairs.csv` |
...
| The public-discussion 2022 Bisan 1-dong one-pair example is confirmed in official data but is not pooled into the Gwangju-Jeonnam five-pair test. | `outputs/public_discussion_claims_audit.csv`, `outputs/public_discussion_claims_audit.json` |
| The 47 core numerical claims in the manuscript match the current outputs. | `outputs/core_claims_verification.csv`, `outputs/core_claims_verification.json` |
| The statistical conclusion is robust across Poisson, exact, sensitivity, and resampling checks. | `outputs/statistical_robustness_audit.csv`, `outputs/statistical_robustness_audit.json` |
| The manuscript and submission documents do not use informal video-platform or subtitle-based evidence. | `outputs/video_source_exclusion_audit.csv`, `outputs/video_source_exclusion_audit.json` |
...
| The manuscript's legal/causal claim boundary and raw-data audit framing pass. | `outputs/claim_boundary_audit.csv`, `outputs/claim_boundary_audit.json` |
