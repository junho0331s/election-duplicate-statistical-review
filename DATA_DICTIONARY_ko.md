...
| `outputs/probability_designated_pairs.csv` | 지정 대응쌍 조건부 확률 시나리오 1개 | 관찰된 광주전남 5개 대응쌍이 사전에 지정된 대응쌍이라고 둘 때 모두 같은 득표쌍이 될 조건부 확률 | `case`, `k_space`, `designated_pairs`, `probability`, `probability_percent`, `reciprocal` |
...
| `outputs/core_claims_verification.csv` | 핵심 검증 명제 1개 | `verify_core_claims.py`가 대조한 47개 핵심 수치를 스프레드시트에서 확인 | `claim`, `expected`, `actual`, `abs_tol`, `status` |
| `outputs/core_claims_verification.json` | JSON 검증 요약 | 같은 47개 핵심 수치의 기계가독 검증 결과 | `status`, `check_count`, `scope`, `checks` |
...
| `outputs/statistical_robustness_audit.json` | JSON 통계 강건성 감사 요약 | 같은 11개 강건성 감사 항목의 기계가독 검증 결과 | `status`, `check_count`, `scope`, `checks` |
...
| `outputs/objection_coverage_audit.json` | JSON 반론 커버리지 감사 요약 | 한 쌍 반론, 2014 포함, 3쌍 대 5쌍, 사전 지정 5쌍 확률 해석, 사후탐색, 이질성, 자기선택, 드문 사건, 공식파일 한계, 대안 설명, 인수인계 원자료 요구, 원자료 공개 후 판정 트리 반론의 커버리지 검증 | `status`, `check_count`, `scope`, `checks` |
...
| 광주전남 5개 대응쌍을 사전 지정된 대응쌍으로 두면 모두 일치할 조건부 확률은 약 \(9.54\times10^{-26}\)이다. | `outputs/probability_designated_pairs.csv` |
...
| 본문 핵심 수치 47개가 현재 산출물과 일치한다. | `outputs/core_claims_verification.csv`, `outputs/core_claims_verification.json` |
